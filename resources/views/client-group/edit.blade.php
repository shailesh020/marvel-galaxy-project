...
@section('content')
    ...
    <div class="card p-3">
        {!! Form::model($group, ['method' => 'PATCH', 'route' => ["$title.update", $group->id],'files'=> true]) !!}
        <div class="row">
                ...
                <div class="form-group">
                    ...
                    <select name="client[]" id="group_id" class="form-control Select2" multiple>
                        @foreach ($clients as $client)
                            <option value="{{ $client->id }}" @if($group && in_array($client->id,json_decode($group->client))){{"selected"}}@endif>
                                {{ $client->name }}
                            </option>
                        @endforeach
                    </select>
